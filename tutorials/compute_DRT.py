...
def compute_L2_aristo(log_tau, sigma):
    ...
    L2_raw = compute_L2(log_tau)
    N_tau = log_tau.size

    # Create a vector (1/sigma, 0, ..., 0)
    top_vector = np.zeros((1, N_tau))
    top_vector[0, 0] = 1/sigma

    # Create a vector (0, 0, ..., 1)
    bottom_vector = np.zeros((1, N_tau))
    bottom_vector[0, -1] = 1/sigma

    out_L2 = np.vstack([top_vector, L2_raw, bottom_vector])
...
# AI Generated. To check.
def compute_L2_one_sided(log_tau, sigma):

    N_tau = log_tau.size
    out_L2 = np.zeros((N_tau, N_tau))

    # Handle left boundary using forward difference
    h1 = log_tau[1]-log_tau[0]
    h2 = log_tau[2]-log_tau[1]
    out_L2[0, 0] = 1/sigma*((2/h1)-(2/(h1+h2)))
    out_L2[0, 1] = 1/sigma*(-(2/h1)+(2/h2))
    out_L2[0, 2] = 1/sigma*((2/(h1+h2))-(2/h2))
    ...
    # Handle right boundary using backward difference
    hN_1 = log_tau[-1]-log_tau[-2]
    hN_2 = log_tau[-2]-log_tau[-3]
    out_L2[-1, -3] = 1/sigma*(-(2/(hN_1+hN_2))+(2/hN_2))
    out_L2[-1, -2] = 1/sigma*((2/hN_1)-(2/hN_2))
    out_L2[-1, -1] = 1/sigma*(-(2/hN_1)+(2/(hN_1+hN_2)))
